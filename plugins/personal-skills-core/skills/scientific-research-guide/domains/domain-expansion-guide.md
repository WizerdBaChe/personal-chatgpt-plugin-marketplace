...
> **Loop pointer (added 2026-09-03).** When a row's verification came from a
> `literature-search-extract` run, the Verification status cell also carries `run:<run_id>`.
> No new column: the loop derives the row's identity key from the Identifier cell at read
> time (`runs.py affected <key> --scan-srg`) and finds the run by the pointer. Rows whose
> Identifier cell yields no key are listed as WARN for the next-touch retrofit (§3.8), never
> blanket-stamped. Design: `the bundled literature-search-extract/loop/schemas contracts` §8.
...
**Generated front-matter card (added 2026-09-02).** Every manifest-registered profile
...
it is untouched and remains the human-facing metadata.
...
2. **Resolve every anchor BEFORE any claim enters a node.** For an AI-packet source: re-derive
   ...
   note only where a specific nameable trigger exists. From 2026-09-03 the resolution itself
...
6. **Wire in and test in the same change, not later**: SKILL.md description keywords,
   `the skill-routing trigger dictionary` entry, and an eval routing case in `evals/evals.json`,
